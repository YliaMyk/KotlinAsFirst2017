...
fun minDivisor(n: Int): Int {
    for (i in 2..round(Math.sqrt(n.toDouble())).toInt()) {
        if (n % i == 0) return i
    }
    return n
}
...
/**
 * функция для двух сложных задач. На вход подаются переремnенная n и функция
 */
fun createRow(n : Int, intFun: (Int) -> String): Int {
    var order = 1
    var condition = 0
    var buf = ""
    while (condition < n) {
 ...
 */
fun fibSequenceDigit(n: Int): Int = createRow(n){b -> "${fib(b)}" }
